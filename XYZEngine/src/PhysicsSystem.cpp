			...
			for (int j = 0; j < colliders.size(); j++)
			{
				...
				if (colliders[i]->bounds.intersects(colliders[j]->bounds, intersection))
				{
					...
					else if (!colliders[i]->isTrigger)
					{
						...
						sf::Vector2f intersectionPosition = { intersection.getPosition().x - 0.5f * intersectionWidth, intersection.getPosition().y - 0.5f * intersectionHeight };

						auto aPosition = colliders[i]->bounds.getPosition();
